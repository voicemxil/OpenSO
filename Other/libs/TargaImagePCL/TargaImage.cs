...
namespace TargaImagePCL
{
...

    {
        ...
        private RawTGABitmap bmpTargaImage = null;
        private RawTGABitmap bmpImageThumbnail = null;
        ...
        {
            return System.Text.Encoding.UTF8.GetString(dat, 0, dat.Length).TrimEnd('\0');
        }
        ...
        private void LoadTGAFooterInfo(BinaryReader binReader)
        {
            ...
            if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
            {
                ...
                try
                {
                    ...
                    string Signature = GetStringFromBytes(binReader.ReadBytes(TargaConstants.FooterSignatureByteLength)).TrimEnd('\0');
                    ...
                    if (string.Compare(Signature, TargaConstants.TargaFooterASCIISignature) == 0)
                    {
                        ...
                        string ResChar = GetStringFromBytes(binReader.ReadBytes(TargaConstants.FooterReservedCharByteLength)).TrimEnd('\0');
            ...
            if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
            {
                try
                {
                    ...
                    if (this.objTargaHeader.ImageIDLength > 0)
                    {
                        ...
                        this.objTargaHeader.SetImageIDValue(GetStringFromBytes(ImageIDValueBytes).TrimEnd('\0'));
                    }
                }
                catch (Exception ex)
                {
            ...
            if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
            {
                ...
                if (this.objTargaFooter.ExtensionAreaOffset > 0)
                {
                    try
                    {
                        ...
                        this.objTargaExtensionArea.SetAuthorName(GetStringFromBytes(binReader.ReadBytes(TargaConstants.ExtensionAreaAuthorNameByteLength)).TrimEnd('\0'));
                        this.objTargaExtensionArea.SetAuthorComments(GetStringFromBytes(binReader.ReadBytes(TargaConstants.ExtensionAreaAuthorCommentsByteLength)).TrimEnd('\0'));
                        ...
                        this.objTargaExtensionArea.SetJobName(GetStringFromBytes(binReader.ReadBytes(TargaConstants.ExtensionAreaJobNameByteLength)).TrimEnd('\0'));
                        ...
                        this.objTargaExtensionArea.SetSoftwareID(GetStringFromBytes(binReader.ReadBytes(TargaConstants.ExtensionAreaSoftwareIDByteLength)).TrimEnd('\0'));


                        // get the version number and letter from file
                        float iVersionNumber = (float)binReader.ReadInt16() / 100.0F;
                        string strVersionLetter = GetStringFromBytes(binReader.ReadBytes(TargaConstants.ExtensionAreaSoftwareVersionLetterByteLength)).TrimEnd('\0');
        ...
        private byte[] LoadImageBytes(BinaryReader binReader)
        {
            ...
            if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
            {
                if (this.objTargaHeader.ImageDataOffset > 0)
                {
                    ...
                    {
                        throw new Exception("something bad happened" + rows.Count);
                    }
        ...
        private TGAPixelFormat GetPixelFormat()
        {
            ...
            TGAPixelFormat pfTargaPixelFormat = TGAPixelFormat.Undefined;

            // first off what is our Pixel Depth (bits per pixel)
            switch (this.objTargaHeader.PixelDepth)
            {
                case 8:
                    pfTargaPixelFormat = TGAPixelFormat.Grayscale_8bpp;
                    break;
                    ...
                    if (this.Format == TGAFormat.NEW_TGA)
                    {
                        switch (this.objTargaExtensionArea.AttributesType)
                        {
                            ...
                            case 2: // no alpha data
                                pfTargaPixelFormat = TGAPixelFormat.RGB555_16bpp;
                                break;

                            case 3: // useful alpha data
                                pfTargaPixelFormat = TGAPixelFormat.ARGB1555_16bpp;
                                break;
                        }
                    }
                    else
                ...
                case 24:
                    pfTargaPixelFormat = TGAPixelFormat.RGB_24bpp;
                    break;
                ...
                case 32:
                    ...
                    if (this.Format == TGAFormat.NEW_TGA)
                    {
                        switch (this.objTargaExtensionArea.AttributesType)
                        {
                            ...
                            case 1:
                            case 2: // no alpha data
                                pfTargaPixelFormat = TGAPixelFormat.RGB_32bpp;
                                break;

                            case 0:
                            case 3: // useful alpha data
                                pfTargaPixelFormat = TGAPixelFormat.ARGB_32bpp;
                                break;
