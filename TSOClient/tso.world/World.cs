...
namespace FSO.LotView
{
    ...
    public class World : _3DScene
    {
        ...
        public void ChangeAAMode(GraphicsDevice gd)
        {
            ...
            // diagnosis could not pin to the inputs (velocity + packed depth were both verified correct
            ...
            // TAA fallback: only substitute FXAA if the TAA effect genuinely failed to load (missing
            // content), not because of backend — GL now runs TAALite via TAAResolve.cs same as DirectX
            // runs the full TAA technique, so backend alone is no longer a reason to fall back.
            ...
            if (State.CameraMode < CameraRenderMode._3D)
                FSO.Common.Utils.TwoDFingerprint.Log(
                    $"world enable3D={FSOEnvironment.Enable3D} useMRT={FSOEnvironment.UseMRT} depthTarget={PPXDepthEngine.Has2DDepthTarget}" +
                    $" msaa={PPXDepthEngine.MSAA} ssaa={PPXDepthEngine.SSAA} lightMode={cfg.LightingMode} advLight={cfg.AdvancedLighting}" +
                    $" dir={cfg.Directional} camMode={State.CameraMode} texCompress={FSOEnvironment.TexCompress}");
