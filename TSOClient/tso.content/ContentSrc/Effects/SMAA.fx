// SMAA.fx — wraps Jorge Jimenez's reference SMAA.hlsl (Copyright (C) 2013 Jimenez et al., MIT) unchanged.
// Passes run by SMAAResolve (C#): EdgeDetect (color->edges), BlendWeights (edges->weights, needs
// AreaTex+SearchTex), NeighborBlend (color+weights->screen). Both UI presets use this PRESET_HIGH build.

// SMAA_HLSL_3 = DX9-style tex2D path; MGFX accepts it at both ps_3_0 and ps_4_0.
#define SMAA_HLSL_3
#define SMAA_PRESET_HIGH

// Our AreaTex is RGBA8 with components in R and G, not SMAA's legacy L8A8 (R, A) — select .rg.
