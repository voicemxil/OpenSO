...
namespace FSO.Patcher
{
    public class CLIPatcher
    {
        ...
        {
            // merged: the transactional FailUpdate path (origin rollback work) with the accurate
            // running-game diagnosis (patch-while-running flow) — this branch fires when the game
            // still holds its files locked after the close-wait.
            FailUpdate("OpenSO is still running, so the update can't be applied — the game holds its files locked. Close OpenSO completely (check for a background process) and run the update again.");
        }
